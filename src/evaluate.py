...
def main():
    ...
    test = pd.read_csv(HOME_DIR+"data/prepared/test.csv")
    
    # processing test dataset
    X_test, X_test_scaled, y_test = preprocess(test)
    
    # loading the model
    model = mlflow.sklearn.load_model(HOME_DIR+"model/artifacts/model")
...
def preprocess(test):
    ...
    scaler = mlflow.sklearn.load_model(HOME_DIR+"model/artifacts/scaler")
...
def model_promotion(MODEL_NAME, X_test, y_test, pred, score):
    ...
    with open(HOME_DIR+"evaluation/deploy_flag.txt", 'w') as outfile:
    ...
    perf_comparison_plot.figure.savefig(HOME_DIR+"evaluation/perf_comparison.png")
